...
sampler2D _MainTex_Mirror; float4 _MainTex_Mirror_ST;
...
{
    float outlineTex;
    if(unity_CameraProjection[2][0]+unity_CameraProjection[2][1])
        outlineTex = tex2Dlod(_OutlineWidthTexture, float4(TRANSFORM_TEX(v.texcoord,_MainTex_Mirror), 0, 0)).r;
    else
        outlineTex = tex2Dlod(_OutlineWidthTexture, float4(TRANSFORM_TEX(v.texcoord, _MainTex), 0, 0)).r;
...
float4 frag_forward(v2f i) : SV_TARGET
{
    ...
    // additive matcap
        ...
        // uv
        float2 mainUv = TRANSFORM_TEX(i.uv0, _MainTex_Mirror);
        ...
        // main tex
        half4 mainTex = tex2D(_MainTex_Mirror, mainUv);
        
        // alpha
        half alpha = 1;
#ifdef _ALPHATEST_ON
        alpha = _Color_Mirror.a * mainTex.a;
        alpha = (alpha - _Cutoff) / max(fwidth(alpha), EPS_COL) + 0.5; // Alpha to Coverage
        clip(alpha - _Cutoff);
        alpha = 1.0; // Discarded, otherwise it should be assumed to have full opacity
#endif
...
#ifdef MTOON_FORWARD_ADD
#else
        half3 emission = tex2D(_EmissionMap_Mirror, mainUv).rgb * _EmissionColor_Mirror.rgb;
        col += lerp(emission, half3(0, 0, 0), i.isOutline);
#endif
